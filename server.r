...
# df_test <- read.csv("./data/df_test.csv", header = TRUE, check.names=FALSE, stringsAsFactors = FALSE, sep = ",", strip.white = TRUE)
############
...
shinyServer(function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    m <- leaflet(data = df_test[,]) %>%
      ...
      addCircles(lng = df_test$LONG, lat = df_test$LAT,color=~df_test$`Color`, radius = ~20*df_test$`Total Tons`, popup = as.character(paste(df_test$`Job Name`,df_test$`Total Tons`, sep="\n")) )
    m
  })
